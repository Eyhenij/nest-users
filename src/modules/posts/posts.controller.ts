...
import { Post as UsersPosts } from './post.model';
...
export class PostsController {
	...
	@ApiResponse({ status: 200, description: 'get array of posts:success', type: [UsersPosts] })
	@ApiResponse({ status: 401, description: 'unauthorized', type: UnauthorizedException })
	public async findAll(@Query('userUUID', new ParseUUIDPipe()) userUUID: string): Promise<UsersPosts[]> {
	...
	@ApiResponse({ status: 200, description: 'get one post:success', type: UsersPosts })
	@ApiResponse({ status: 401, description: 'unauthorized', type: UnauthorizedException })
	...
	@ApiResponse({ status: 201, description: 'create post:success', type: ResponseMessageDto })
	@ApiResponse({ status: 401, description: 'unauthorized', type: UnauthorizedException })
	@ApiBody({ type: CreatePostDto })
	public async create(@Body() createPostData: CreatePostDto): Promise<ResponseMessageDto> {
